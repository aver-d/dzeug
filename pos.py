...
def new_german_postagger(stanford_dir, fast=False):
  ...
  return StanfordPOSTagger(str(modelfile), jar)
...
def lemma_adj(lemmas, word, begin_sent=False):
  ...
  return lemma if word.islower() or begin_sent else\
         lemma.capitalize() # catch eg. Aachener, person from Aachen
...
def reconstruct(sent):
  ...
  def combine(tag1, tag2):
    ...
    return tag1.word + ' ' if space else\
           tag1.word
...
def read_input(paths, lang_check):
   return concat_documents(paths, lang_check) if paths else\
          (read_stdin(lang_check), None)
...
"For each named file provided, save its corresponding parsed document. The file is saved in 'the directory given by the config file's 'parsed' option. The filename is saved with an additional .dzeug extension. Default: output is sent to stdout and input files are separated with two empty lines.")
  @option('--no-lang-check', 'no_lang_check', is_flag=True, help='Skip language detection')
