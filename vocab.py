...
def valid_word(word):
  return len(word) > 1 and not word.isupper() and\
...
def commandline():
  ...
  @option('--nouns', '-n', is_flag=True, help="Include nouns")
  @option('--verbs', '-v', is_flag=True, help="Include verbs")
  @option('--adjectives', '-a', is_flag=True, help="Include adjectives")
  @option('--range', '-r', callback=click_validate_range(1, 45000), help='Limit vocab rank to this range [1,45000]')
  @option('--simple', '-s', is_flag=True, help="Show only lemmas as simple list, no formatting")
  @option('--tagged', '-t', is_flag=True, help="Input text is already tagged in dzeug format ")
  def cli(path, range, nouns, verbs, adjectives, simple, tagged):
    ...
    doc = read_parsed(text) if tagged else\
          tag(text)
