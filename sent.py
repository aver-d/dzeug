...
def format_normal(wrap, sentence):
  result = '- ' + '\n  '.join(wrap(sentence))
  result = result.replace(markleft, orange)\
                 .replace(markright, gray)
...
def ask_misspelling(word, choices):
  ...
  return choices[n-1] if valid else\
         None
...
def cache_check():
  ...
  if cache_stale(cache_path(), files):# or True:
...
def commandline():
  ...
  @option('--centre',   '-c', is_flag=True, help='Align word centrally when displaying')
  @option('--sentences','-s', is_flag=True, help="Show only sentences, no document titles")
  @option('--stdout',   '-t', is_flag=True, help='No pager, send directly to terminal')
  @option('--shuffle',  '-z', 'shuf', is_flag=True, help='Shuffle results order')
  @option('--no-retry', 'no_retry', is_flag=True, help='Exit immediately on not found')
  def cli(word, paths, centre, sentences, stdout, shuf, no_retry):
    ...
    paths = config.parsed_dir if not paths else\
            files_only(paths)

    termwidth, _ = get_terminal_size()
    wrap = textwrap.TextWrapper(width=min(77, termwidth-1)).wrap

    format = partial(format_centre, termwidth) if centre else\
             partial(format_normal, wrap)
